...
\begin{itemize}
  ...
  \item \textbf{Utils:} Classe di utilità con metodi statici condivisi da tutti i sotto-sistemi per la validazione degli input (email, password, nome, cognome, codice fiscale, telefono), l'hashing delle password (SHA-256), e la generazione di token univoci, codici di sblocco e OTP.
...
\begin{itemize}
  \item \textbf{EntityStudente:} Classe di modello che rappresenta lo studente autenticato sulla piattaforma. Contiene i dati anagrafici e di accesso dell'utente (nome, cognome, codice fiscale, email, password hashata, telefono, percorso della foto) e la lista delle voci di background artistico (datiBackground). È l'unica classe entity del sistema, in quanto è l'unico oggetto persistito che viene caricato in memoria durante la sessione attiva.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{FormIdentitaDigitale:} Riproduce la pagina di autenticazione del Provider di identità digitale. Contiene i campi per l'inserimento di nome utente e password e il pulsante: Conferma.
...
\begin{itemize}
  ...
  \item \textbf{SchermataBackgroundArtistico:} Mostra le voci del background artistico dello studente e consente di gestirle. Contiene i pulsanti: Aggiungi Dati, Modifica Dati ed Elimina Dati.
  ...
  \item \textbf{SchermataPrivacy:} Consente di impostare il livello di privacy di un contenuto. Contiene gli elementi per la selezione della tipologia di privacy (Pubblico, Privato, Non in elenco) e il pulsante: Conferma.
...
\begin{itemize}
  \item \textbf{SelezionaContenutiControl:} Gestisce il caso d'uso Selezione Contenuti (UC 4.1). Coordina il recupero di tutti i contenuti dello studente, indipendentemente dalla privacy (pubblici e privati), e li presenta per la selezione, interrogando il database tramite DatabaseManager.
  \item \textbf{GestisciLinkControl:} Gestisce i casi d'uso Genera Link (UC 4.2), Visualizza Elenco Link (UC 4.3) e Revoca Link (UC 4.4). Coordina la generazione del token univoco del link e il calcolo della data di scadenza (14 giorni), il recupero dei link generati dallo studente con il relativo stato di visualizzazione e la revoca anticipata tramite DatabaseManager.
  \item \textbf{SelezionaContenutiPrivatiControl:} Gestisce il caso d'uso Selezione Contenuti Privati (UC 7.1). Coordina il recupero dei soli contenuti privati dello studente e li presenta per la selezione, interrogando il database tramite DatabaseManager.
  \item \textbf{GestisciCodiceControl:} Gestisce i casi d'uso Genera Codice (UC 7.2), Visualizza Elenco Codici (UC 7.3) e Revoca Codice (UC 7.4). Coordina la generazione del codice alfanumerico univoco, il recupero dei codici generati dallo studente e la loro revoca tramite DatabaseManager.
\end{itemize}
...
Contiene le classi relative all'esecuzione di funzionalità automatiche attivate dall'attore Tempo. Non è presente alcuna interfaccia grafica destinata all'utente: il package interface contiene unicamente la classe boundary non grafica BoundaryTempo, che rappresenta l'interfaccia verso l'attore Tempo, mentre il package control contiene ControlValiditaLink.

\begin{itemize}
  \item \textbf{BoundaryTempo:} Rappresenta l'interfaccia verso l'orologio di sistema (attore Tempo), tramite cui il Sistema ottiene la data e l'ora correnti necessarie alla verifica della validità dei link di condivisione.
  \item \textbf{ControlValiditaLink:} Gestisce il caso d'uso Verifica Validità Link (UC 4.5). Viene attivata automaticamente dall'attore Tempo come processo batch giornaliero alle ore 00:00; ottiene la data corrente tramite BoundaryTempo e interroga il database tramite DatabaseManager per identificare tutti i link di condivisione il cui stato è ATTIVO ma la cui data di scadenza è antecedente alla data odierna, aggiornandone lo stato in SCADUTO.
\end{itemize}
