...
\begin{itemize}
  ...
  \item \textbf{SessioneCorrente:} Singleton che mantiene il riferimento all'oggetto Studente autenticato durante la sessione attiva. Viene popolato al completamento del login e azzerato al logout.
...
\begin{itemize}
  ...
  \item \textbf{SessioneCorrente:} Singleton che mantiene in memoria il riferimento all'oggetto EntityStudente autenticato durante la sessione corrente. Viene popolato al completamento del login e azzerato al logout.
...
Il package \texttt{utility} comprende inoltre il sotto-package \texttt{dto}, che raccoglie i Data Transfer Object (record Java immutabili) con cui il DatabaseManager restituisce ai control i soli dati necessari alle interfacce grafiche, senza esporre le entità di persistenza:

\begin{itemize}
  \item \textbf{DatiStudente:} Dati essenziali di uno studente (id, nome, cognome), utilizzati per i risultati di ricerca nella consultazione.
  \item \textbf{DatiFile:} Metadati di un contenuto (id, titolo, descrizione, tipologia, stato di privacy, percorso fisico e categoria di appartenenza).
  \item \textbf{DatiLink:} Dati di un link di condivisione (id, URL/token, data di scadenza, indicatore di avvenuta visualizzazione e stato).
  \item \textbf{DatiCodice:} Dati di un codice di sblocco (id, codice alfanumerico e stato attivo/revocato).
  \item \textbf{DatiCategoria:} Dati di una categoria artistica (id e nome).
  \item \textbf{DatiBackground:} Dati di una voce del background artistico (id, scuola/università, collaborazioni effettuate, autori delle collaborazioni e partecipazioni).
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{util:} Contiene classi e interfacce per la gestione di collezioni (List, Map) e la generazione di valori casuali (UUID, Random).
